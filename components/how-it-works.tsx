...
export default function HowItWorks() {
  ...
  return (
    <section ref={sectionRef} className="py-16 md:py-24 bg-background transition-colors duration-300">
            ...
            return (
              <div key={index} className="relative">
                {index < steps.length - 1 && (
                  <div className="hidden md:block absolute top-16 left-[50%] w-[calc(100%-2rem)] h-0.5 bg-border" />
                )}

                <div className={`relative bg-card rounded-xl p-6 text-center shadow-sm border border-border transition-all duration-700 ${
                  isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
                }`}
